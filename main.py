...
def load_data():
    ...
    with Manager() as manager:
        ...
        if now - last_daily_run >= timedelta(hours=23): # and now.strftime("%H") == '00':
            ...
            d7_data = []
            temp_data = []
            for i in range(len(organized_data['icon'])//6-7): #4 6
                for j in range(12):
                ...
                temp_data = []
            for i in range(len(organized_data['icon'])//6-7,len(organized_data['icon'])//6-5): #4,6 6,8
                for j in range(4):
                    temp_data.append(organized_data['icon'][12*(len(organized_data['icon'])//6-7)+(i-6)*4+j][1]) #48 72
                counter = Counter(temp_data)
                freq = counter.most_common(1)[0][1]
                modes = [key for key, count in counter.items() if count == freq]
                d7_data1.append(modes[0])
